...
- **Internal linking:** Link to other pages/posts on the same site. **Every internal link must be verified before the post is shipped.** URLs come from `{{ title | slugify }}`, not the filename. Eleventy's slugify has quirks: apostrophes become `-` (e.g. `What's` -> `what-s`, not `whats`), colons and question marks are stripped. Do NOT slugify titles mentally. Verify by listing `_site/` (e.g. `ls _site | grep <keyword>`) or by grepping existing posts for how they link to the same target. If the build is stale, rebuild first.
...
- **Link check before shipping:** After creating or editing a post, extract every link (internal and external) and verify each one. A post is not done until every link resolves 200. This is non-negotiable; broken links have shipped repeatedly and damage the site.
- **Build check before shipping (NON-NEGOTIABLE):** After creating or editing a post, run `npm run build` locally and confirm it completes with no YAML, parser, or template errors. A post is not done until the build passes. Silent frontmatter or template errors have shipped repeatedly and broken the deploy.
- **YAML frontmatter colons:** Any `description`, `title`, or `seoTitle` value that contains a colon (`:`) MUST be wrapped in double quotes, or YAML treats the colon as a nested mapping and the build fails. Rule of thumb: if the value contains `:`, `#`, `-` at start, or any `{}[]&*|>` character, wrap it in `"..."`. When in doubt, quote it.
